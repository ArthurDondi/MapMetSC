...
Next, we read the manually assigned celltype and metacluster labels for the AD clusters. These are filled in externally into the `cluster_annotation_template.xlsx` written above (based on thorough examination of images and co-expressions and using literature) and saved as `cluster_annotation_completed.xlsx` in the output folder.
Let's start with celltype assignments...

```{r annotations-celltypes}
file_path <- file.path(params$output, 'cluster_annotation_completed.xlsx')
...
col_celltype <- c(
  ...
  "other (CXCR4+)" = rgb(150, 150, 150, maxColorValue = 255),  # mid grey
  "CXCR4+ cells"   = rgb(195, 195, 195, maxColorValue = 255)   # light grey
)

...
## Preprocess adrenal gland (AD) files to remove supplementary channels

The adrenal gland (AD) intensity files contain supplementary negative-control channels (`IF4_NEGATIVE_MIX`) that are not part of the marker panel. We remove these columns from the intensity CSVs so that the files match the panel before reading them in. This step overwrites the CSV files in place and is idempotent (re-running it on already-cleaned files has no effect).

```{r preprocess-ad-channels}

...
This step overwrites the intensity CSVs in place, so it can only be run where the data is **writable**. Run it manually, once, from the parent folder of the `intensities-*` folders on a writable copy of the data (not against a read-only mounted `input_AD`). Once the files are cleaned, the pipeline below reads them normally.

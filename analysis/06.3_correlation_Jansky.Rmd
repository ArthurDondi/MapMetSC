...
# ================================
plot_dir <- file.path(params$output, "plots/06.3_correlation_Jansky")
...
protein <- protein[, protein$metacluster == "tumor" | protein$celltype == "Schwann cells"]
...
Next, we will set common markers between the reference and query dataset. Therefore, we have to rename the features in the imaging dataset to the names of the corresponding genes. A csv files with protein-gene mappings can be obtained from Zenodo [(protein2gene.csv)](https://doi.org/10.5281/zenodo.13220634)
...
save_plot(h_rna, "Jansky_rna_mean_per_cluster_heatmap.pdf", width = 8, height = 8)
...
p_umap_rna <- DimPlot(rna, reduction="umap", cols=col_rna_clusters) +
  # Harmonised square panel border (all UMAPs).
  theme(axis.line    = element_blank(),
        panel.border = element_rect(colour = "black", fill = NA, size = 0.5))

save_plot(p_umap_rna, "Jansky_rna_umap.pdf", width = 8, height = 6)
...
```{r mean-per-cluster-combined, fig.height=10, fig.width=12}
...
column_ha = HeatmapAnnotation(modality=modality,
                              ncells=anno_barplot(cell_count, height = unit(1, "cm")),
...
lgd_list = list(
    Legend( labels = c(0, 25, 50, 75, 100), title = "percentage expressed",
            graphics = list(
...
hp <- Heatmap(exp_mat,
        ...
        column_names_rot = 45,
        column_names_max_height = max_text_width(colnames(exp_mat), gp = gpar(fontsize = 10)),
        border = "black")

d1 <- draw( hp, annotation_legend_list = lgd_list, padding = unit(c(8, 8, 2, 2), "mm"))

save_plot(hp, "Jansky_dotplot_expression_percent.pdf", width = 12, height = 10, annotation_legend_list = lgd_list, padding = unit(c(8, 8, 2, 2), "mm"))
```
...
save_plot(pca, "Jansky_pca_meanexpression.pdf", width = 10, height = 8)

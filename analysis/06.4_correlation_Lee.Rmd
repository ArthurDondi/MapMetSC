...
p1 <- dittoDimPlot(protein, var = "celltype_predicted",
             ...
             order = "randomize",
             legend.show=TRUE) +
  # "tumor"/"other" are PT/BM background labels (not Lee reference celltypes):
  # keep them plotted as background but drop them from the legend, which then
  # shows only the Lee-predicted TME celltypes.
  scale_color_manual(values = metadata(protein)$color_vectors$col_predicted_label,
                     breaks = setdiff(sort(unique(protein$celltype_predicted)),
                                      c("tumor", "other"))) +

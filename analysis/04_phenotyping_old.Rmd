...
df_counts <- colData(spe) %>%
  as_tibble() %>%
  mutate(is_target = tissue == "BM" & metacluster == "tumor") %>%
  group_by(fm_id) %>%
  summarise(n_cells = sum(is_target), .groups = "drop")
...
ggplot(df_counts, aes(x = fm_id, y = n_cells)) +
  ...
  labs(
    x = "fm_id",
    y = "Number of tumor cells in BM",
    title = "Tumor cell counts per fm_id (BM only)"
  ) +

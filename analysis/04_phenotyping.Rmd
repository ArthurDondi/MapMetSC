...
pca_mat <- HarmonyMatrix(
  ...
  vars_use  = c("fm_id", "tissue", "stainer", "imc_date"),
  do_pca    = FALSE
)
...
## Batch correction strength: Average Silhouette Width
We quantify the strength of batch effect correction using the **average silhouette width (ASW)** of batch labels computed in PCA space. A lower (or more negative) ASW after Harmony indicates better batch integration: cells from different batches are more interleaved.

```{r batch-correction-asw, fig.width=5, fig.height=5}
# Subsample for computational feasibility
set.seed(221228)
n_sample <- min(5000, ncol(spe))
...
dist_post <- dist(reducedDim(spe, "PCA_clustering")[idx, 1:10])

# Use stainer as batch variable (clear technical batch)
batch_labels <- as.integer(factor(spe$stainer[idx]))
...
p_asw <- ggplot(asw_df, aes(x = Stage, y = ASW, fill = Stage)) +
  geom_col(width = 0.5) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  scale_fill_manual(values = c("Pre-Harmony" = "#E88C8C", "Post-Harmony" = "#8CBBE8")) +
  labs(
    title = "Average Silhouette Width of stainer labels\nbefore and after Harmony correction",
...
save_plot(p_asw, "batch-correction-asw.png", width = 5, height = 5)
p_asw
```

...
```{r umap-harmony-8panel, fig.width=24, fig.height=12}
...
save_plot(panel_8, "UMAP-harmony-8panel.png", width = 24, height = 12)
panel_8
```
...
```{r batch-correction-asw, fig.width=5, fig.height=5}
# Subsample for computational feasibility
set.seed(221228)
n_sample <- min(20000, ncol(spe))
idx <- sample(seq_len(ncol(spe)), n_sample)

# Compute Euclidean distances in pre- and post-Harmony PCA spaces (top 10 PCs)
dist_pre  <- dist(pca_mat_raw[idx, 1:10])
dist_post <- dist(pca_mat[idx, 1:10])

# Define covariates to loop over
covariates <- c("fm_id", "tissue", "stainer", "imc_date", "pg_clusters", "pg_clusters_noharmony")
...
for (cov in covariates) {
  # Get batch labels for current covariate
  batch_labels <- as.integer(factor(spe[[cov]][idx]))
  
  # Compute silhouette widths
  sil_pre  <- silhouette(batch_labels, dist_pre)
  sil_post <- silhouette(batch_labels, dist_post)
  
  # Compute average silhouette widths
  asw_pre  <- mean(sil_pre[, "sil_width"])
  asw_post <- mean(sil_post[, "sil_width"])
  
  # Create data frame for plotting
  asw_df <- data.frame(
    Stage = factor(c("Pre-Harmony", "Post-Harmony"), levels = c("Pre-Harmony", "Post-Harmony")),
    ASW   = c(asw_pre, asw_post)
  )
  
  # Create plot
  p <- ggplot(asw_df, aes(x = Stage, y = ASW, fill = Stage)) +
    geom_col(width = 0.5) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
    scale_fill_manual(values = c("Pre-Harmony" = "#E88C8C", "Post-Harmony" = "#8CBBE8")) +
    labs(
      title = paste("ASW by", cov),
...
# Combine plots into a panel using patchwork
p_panel <- wrap_plots(p_list, ncol = 2)

# Save the combined panel
save_plot(p_panel, "sample-batch-correction-asw-panel.png", width = 10, height = 10)
...
file_path <- file.path(params$input, 'completed_cluster_annotation_template.xlsx')
...
cluster_names <- setNames(annotation$BM_celltype, annotation$cluster)
celltypes <- recode(spe$pg_clusters, !!!cluster_names)
spe$celltype <-  as.vector(celltypes)

metacluster_names <- setNames(annotation$BM_metacluster, annotation$cluster)
...
col_celltype_new <- c(
  ...
  "GD2hi TC" = rgb(27,158,119, maxColorValue = 255),        # reused from CD44+ TC
  "GD2+ TC" = rgb(102,194,165, maxColorValue = 255),       # reused from GD2lo TC
  "PRPHhi TC" = rgb(65,208,57, maxColorValue = 255),       # reused from GATA3hi TC
  "LUMhi TC" = rgb(60,119,97, maxColorValue = 255),        # reused from CD24- marker-lo TC
  "marker-lo TC" = rgb(230,245,201, maxColorValue = 255),  # reused from CD24+ marker-lo TC
  "ELAVL4hi GATA3hi SOX10hi TC" = rgb(204,235,197, maxColorValue = 255),
  "SOX10hi ELAVL4hi GATA3hi TC" = rgb(198,255,72, maxColorValue = 255),
  "CD56+ TC" = rgb(174,129,37, maxColorValue = 255),       # reused from MSC (kept greenish tone)

  ## ---- T cells (pink/purple tones) ----
  "CD4+ naïve T cell" = rgb(242,3,137, maxColorValue = 255),
  "CD8+ naïve T cell" = rgb(188,128,189, maxColorValue = 255),
  "CD4+ PD1+ T cell" = rgb(254,142,175, maxColorValue = 255),
  "CD8+ GZMB+ PD1lo T cell" = rgb(251,154,153, maxColorValue = 255),
  "FOXP3+ T cell" = rgb(145,2,144, maxColorValue = 255),   # reused from dense T cell region

  ## ---- Myeloid / granulocytes (blue/grey family) ----
  "CD14- PDL1- MO/DC" = rgb(39,130,187, maxColorValue = 255),
  "CD14+ CD11c+ MO" = rgb(51,180,180, maxColorValue = 255), 
  "MPOhi HLA-DR+ DC" = rgb(166,206,227, maxColorValue = 255),

  "MPOhi CD15lo CD11b- granu." = rgb(2,0,138, maxColorValue = 255),
  "MPOhi CD15hi CD11bhi granu." = rgb(113,113,113, maxColorValue = 255),
  "MPOhi CD15hi CD11blo granu." = rgb(169,189,210, maxColorValue = 255),
  "MPOlo CD15lo CD11bhi granu." = rgb(217,217,217, maxColorValue = 255),
  "MPOlo CD15hi CD11blo granu." = rgb(128,177,211, maxColorValue = 255),
  ...
  ## ---- Other ----
  "AD" = rgb(255,255,209, maxColorValue = 255)  # reused from "other"
)

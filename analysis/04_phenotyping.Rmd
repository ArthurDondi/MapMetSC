...
library(circlize)
library(readxl)
#library(writexl)   # only needed to (re)generate the cluster-annotation template; commented out so writexl is not required
library(ggplot2)
library(patchwork)
library(harmony)
library(batchelor)
...
```{r read-data}
spe <- readRDS(file.path(params$output,"spe_QC_2.rds"))
...
# 4. Write Excel
# -----------------------------
# The cluster-annotation template is generated once and then annotated manually
# (assignments published on Zenodo as a Supplementary Table, read below). The
# write step is left commented out so the pipeline does not depend on `writexl`.
# writexl::write_xlsx(
#   annotation_template,
...
col_celltype <- c(
  ...
  ## ---- Tumor ----
  "Ki67+ Tu"     = rgb(128, 128,   0, maxColorValue = 255),   # olive (dark yellow-green)
  ...
  "dense T cell region" = rgb(199,  21, 133, maxColorValue = 255),   # magenta / medium violet red
...
saveRDS(spe, file.path(params$output, "spe_phenotyping.rds"))

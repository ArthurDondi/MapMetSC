...
plot_cluster_bubble <- function(cluster_col, save_name,
  ...
  col_clusters <- existing_colors[clusters]

  # Use caller-supplied color vector if provided, otherwise use auto-generated
  col_for_annot <- if (!is.null(col_annotation)) {
    setNames(list(col_annotation), cluster_col)
  } else {
    setNames(list(col_clusters), cluster_col)
  }

  pt_info <- make_barplot_info(pt_count)
  bm_info <- make_barplot_info(bm_count)
  cluster_annotation_list <- setNames(
    list(factor(clusters, levels = clusters)),
    cluster_col
  )

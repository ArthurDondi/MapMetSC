...
plot_cluster_bubble <- function(cluster_col, save_name,
  ...
  if (annotation_level == "metacluster") {
    ...
    mean_metaclust <- aggregate(
      colData(spe)[[metacluster_col]],
      list(cluster = cluster_vec),
      max
    )
    meta_by_cluster <- setNames(
      as.character(mean_metaclust$x),
      as.character(mean_metaclust$cluster)
    )
  ...
  annot_levels <- if (annotation_level == "metacluster") {
    levels_from_colors <- names(annot_colors)
    if (!is.null(levels_from_colors)) {
...
plot_cluster_bubble(
  ...
  save_name = "bubbleplot-celltypes_raw_k30.pdf",

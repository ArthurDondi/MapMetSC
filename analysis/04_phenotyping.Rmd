...
Before applying Harmony batch correction, we compute a UMAP embedding from the raw PCA space. This allows us to visualize the batch structure present in the data prior to correction.
...
```{r umap-pre-harmony-batch, fig.width=18, fig.height=6}
p_pre_tissue   <- dittoDimPlot(spe, var = "tissue",
                   reduction.use = "UMAP_preharmony", size = 0.3) +
  ggtitle("Pre-Harmony: tissue")

p_pre_stainer  <- dittoDimPlot(spe, var = "stainer",
                   reduction.use = "UMAP_preharmony", size = 0.3) +
  ggtitle("Pre-Harmony: stainer")

p_pre_imc_date <- dittoDimPlot(spe, var = "imc_date",
                   reduction.use = "UMAP_preharmony", size = 0.3) +
  ggtitle("Pre-Harmony: imc_date")

combined_pre <- p_pre_tissue + p_pre_stainer + p_pre_imc_date
save_plot(combined_pre, "UMAP-preharmony-batch.png", width = 18, height = 6)
combined_pre
```
...
## Harmony effect: side-by-side comparison and batch correction strength
To assess the effect of Harmony correction, we compare UMAP embeddings before and after correction, coloured by the same batch variables. We also quantify the strength of batch effect correction using the **average silhouette width (ASW)** of batch labels computed in PCA space. A lower (or more negative) ASW after Harmony indicates better batch integration: cells from different batches are more interleaved.

```{r harmony-comparison, fig.width=18, fig.height=6}
p_post_tissue   <- dittoDimPlot(spe, var = "tissue",
                    reduction.use = "UMAP", size = 0.3) +
  ggtitle("Post-Harmony: tissue")

p_post_stainer  <- dittoDimPlot(spe, var = "stainer",
                    reduction.use = "UMAP", size = 0.3) +
  ggtitle("Post-Harmony: stainer")

p_post_imc_date <- dittoDimPlot(spe, var = "imc_date",
                    reduction.use = "UMAP", size = 0.3) +
  ggtitle("Post-Harmony: imc_date")

combined_post <- p_post_tissue + p_post_stainer + p_post_imc_date
save_plot(combined_post, "UMAP-postharmony-batch.png", width = 18, height = 6)
combined_post
```

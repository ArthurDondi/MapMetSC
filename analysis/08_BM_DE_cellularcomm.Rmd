...
sce_comp$sample_id <- sce_comp$fm_id
...
df_list <- lapply(unique(sce_comp$fm_id), function(x) {
  as.data.frame(t(counts(sce_comp[, sce_comp$fm_id == x])))
})
names(df_list) <- unique(sce_comp$fm_id)

# experiment info
experiment_info <- metadata(sce_comp)$experiment_info

# REQUIRED: create sample_id and align with df_list
experiment_info$sample_id <- experiment_info$fm_id
experiment_info <- experiment_info[match(names(df_list), experiment_info$sample_id), ]

# sanity check (must be TRUE)
stopifnot(all(names(df_list) == experiment_info$sample_id))

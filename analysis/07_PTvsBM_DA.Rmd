...
# ================================
plot_dir <- file.path(params$output, "plots/07_PTvsBM_DA")
...
if(analysis=="paired"){
  ...
  counts <- table(spe$fm_id[spe$metacluster == "tumor" & spe$timepoint == "DE"])
...
sample_to_short <- setNames(
  paste0(as.character(sce$fm_id), "_", as.character(sce$tissue)),
  as.character(sce$sample_id)
)
...
save_plot(p, "SuppplFigS2d2_BM_abundance_tumor.png", width = 16, height = 12)
...
save_plot(p_boxplot, "Fig5c_PTvsBM_boxplot.png", width = 16, height = 12)
```
...
p_pie <- ggplot(df_normalized, aes(x="", y=normalized_count, fill=cluster_id, group=cluster_id)) +
...
save_plot(p_pie, "Fig5a_PTvsBM_piechart.png", width = 8, height = 4)

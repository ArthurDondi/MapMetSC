...
# ---- Hack: add root stubs ----
# The cross-community root segment is filtered out. To imply that the two
# sub-trees connect, we add a vertical stub above each community's sub-root.
# Strategy: find the highest *horizontal* segment in each community (the
# sub-root merge, where y == yend); get its x midpoint; draw a vertical line
# from that height up to the overall root height h_root.
h_root <- max(c(segs$y, segs$yend))

make_root_stub <- function(comm) {
  s     <- segs_c[segs_c$community == comm, ]
  # horizontal segments have y == yend (within floating-point tolerance)
  s_h   <- s[abs(s$y - s$yend) < 1e-6, ]
  if (nrow(s_h) == 0) return(NULL)
  sub   <- s_h[which.max(s_h$y), ]
  x_mid <- (sub$x + sub$xend) / 2
  data.frame(x = x_mid, y = sub$y, xend = x_mid, yend = h_root,
             community = factor(comm))
}

root_stubs     <- rbind(make_root_stub(1), make_root_stub(2))
segs_with_root <- rbind(segs_c, root_stubs)

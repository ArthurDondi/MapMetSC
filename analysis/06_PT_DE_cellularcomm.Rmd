...
# ---- Hack: add root connection ----
# Each community gets a horizontal arm at root height extending to the far
# panel edge (max + 0.5 / min - 0.5), implying the two communities connect
# at the top of the dendrogram.
root_y_val  <- max(segs$yend)
c1_root_seg <- segs_c[segs_c$community == 1 & abs(segs_c$yend - root_y_val) < 1e-6, ]
c2_root_seg <- segs_c[segs_c$community == 2 & abs(segs_c$yend - root_y_val) < 1e-6, ]

if (nrow(c1_root_seg) > 0 && nrow(c2_root_seg) > 0) {
  if (mean(c1_x) < mean(c2_x)) {
    # c1 is on the left: arm extends rightward to panel edge
    # c2 is on the right: arm extends leftward to panel edge
    root_arms <- data.frame(
      x    = c(c1_root_seg$x[1], min_c2 - 0.5),
      y    = c(root_y_val,        root_y_val),
      xend = c(max_c1 + 0.5,     c2_root_seg$x[1]),
      yend = c(root_y_val,        root_y_val),
      community = factor(c(1L, 2L))
    )
  } else {
    # c1 is on the right: arm extends leftward to panel edge
    # c2 is on the left:  arm extends rightward to panel edge
    root_arms <- data.frame(
      x    = c(min_c1 - 0.5,     c2_root_seg$x[1]),
      y    = c(root_y_val,        root_y_val),
      xend = c(c1_root_seg$x[1], max_c2 + 0.5),
      yend = c(root_y_val,        root_y_val),
      community = factor(c(1L, 2L))
    )
  }

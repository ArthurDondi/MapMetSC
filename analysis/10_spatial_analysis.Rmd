...
```{r read-data}
spe <- readRDS(file.path(params$input_docker,"spe_final_clustering.rds"))
...
save_plot(p, "Fig4d1_spatial_neighborhood_clustering.png", width = 8, height = 4)
...
save_plot(p2, "Fig4d2_spatial_neighborhood_clustering_dendogram.png", width = 8, height = 4)
```
...
```{r catalyst-comp-object3, fig.width=12, fig.height=5}
cells <- "tumor"
compare <- "core-border"

if (cells =="immune"){
  order <- c("T cells", "B cells", "MO/DC", "granulocytes")
  keep_cols <- spe$metacluster=="T cells" | spe$metacluster=="B cells" | spe$metacluster=="MO/DC" | spe$metacluster=="granulocytes"
  color <- metadata(spe)$color_vectors$col_celltype_2
}else{
  order <- c("marker-lo Tu","CD24hi Tu","Tu","SOX10+ Tu","LUM+ Tu","CHGA+ Tu","CXCR4+ Tu","Ki67+ Tu","bridge like Tu")
...
```{r visualize-da, fig.width=12, fig.height=4}
p <- plotAbundances(sce, k = "celltype", by = "sample")
...
for (compare in comparisons_list) {
  ...
  save_plot(
    p_abund,
    paste0("Fig4k_abundance_", compare, "_", cells, ".png"),
    width = 12,
    height = 6
  )

...
# Initialize storage for paired DA results (used by Fig4m downstream)
da_results_compartments <- list()

for (cells in c("tumor", "immune")) {
for (compare in comparisons_list) {

  message("Running comparison: ", compare, " (", cells, ")")
  ...
  metadata(sce)$experiment_info <- as.data.frame(colData(sce)) %>%
    dplyr::group_by(sample_id, condition, sample, fm_id) %>%
  ...
  # Filter to fm_ids present in BOTH compartments (valid pairs only)
  fm_ids_comp1 <- experiment_info$fm_id[experiment_info$condition == comps[1]]
  fm_ids_comp2 <- experiment_info$fm_id[experiment_info$condition == comps[2]]
  paired_fmids <- intersect(as.character(fm_ids_comp1), as.character(fm_ids_comp2))
  experiment_info <- experiment_info[experiment_info$fm_id %in% paired_fmids, ]
  # Re-match df_list to the filtered, ordered experiment_info
  # (all sample_ids must be present in df_list since experiment_info was derived from it)
  idx <- match(as.character(experiment_info$sample_id), names(df_list))
  stopifnot(!anyNA(idx))
  df_list <- df_list[idx]

  experiment_info$condition <- factor(experiment_info$condition, levels = comps)
  experiment_info$fm_id <- factor(experiment_info$fm_id)
  ...
  # ---- DA analysis (paired within fm_id) ----
  # Include fm_id as a fixed-effect blocking factor so comparisons are within-image
  design <- createDesignMatrix(experiment_info, cols_design = c("condition", "fm_id"))

  # logFC = comps[2] - comps[1]; fm_id columns follow the condition coefficient
  contrast <- createContrast(c(0, 1, rep(0, length(levels(experiment_info$fm_id)) - 1)))
  ...
  # Store paired DA results for downstream use in Fig4m
  da_results_compartments[[paste0(cells, "_", compare)]] <- res
...
for (cells in cells_list) {
  ...
  for (cluster_name in order) {
    ...
    stats_df <- lapply(comparison_pairs, function(pair) {
      g1 <- pair[1]
      g2 <- pair[2]
      # Look up p-values from the paired diffcyt DA model (fm_id-blocked)
      compare_key <- paste0(cells, "_", g1, "-", g2)
      res_da <- da_results_compartments[[compare_key]]

      if (!is.null(res_da) && cluster_name %in% rownames(res_da)) {
        p_adj_val <- res_da[cluster_name, "p_adj"]
        logFC_val <- res_da[cluster_name, "logFC"]
      } else {
        p_adj_val <- NA_real_
        logFC_val <- NA_real_
      }

      data.frame(group1 = g1, group2 = g2, p_adj = p_adj_val, logFC = logFC_val)
    }) %>% dplyr::bind_rows()
  ...
  # p_adj comes from the paired diffcyt model (BH-adjusted across all clusters within
  # each individual comparison run, i.e. per comparison pair)

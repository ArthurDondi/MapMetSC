...
```{r read-data}
spe <- readRDS(file.path(params$input_docker,"spe_final_PTBM.rds"))
...
spe[,spe$metacluster=="T cells" | spe$metacluster=="B cells" | spe$metacluster=="granulocyte" | spe$metacluster=="MO/DC"]$celltype_3 <- "immune"
...
for (compare in comparisons_list) {
  ...
  if (cells =="immune"){
    ...
    order <- c("marker-lo Tu","CD24hi Tu","Tu","SOX10+ Tu","LUM+ Tu",
               "CHGA+ Tu","CXCR4+ Tu","Ki67+ Tu","bridge like Tu")
    keep_cols <- spe$metacluster == "tumor"
    color <- metadata(spe)$color_vectors$col_celltype_2
  }
  ...
  save_plot(
    p_abund,
    paste0("abundance_", compare, "_", cells, ".png"),
    width = 12,
    height = 6
  )
  ...
  save_plot(
    p_box,
    paste0("boxplot_", compare, "_", cells, ".png"),
    width = 12,
    height = 8
  )
  ...
  save_plot(
    p_bar,
    paste0("barplot_", compare, "_", cells, ".png"),
    width = 12,
    height = 6
  )
}
...
save_plot(
  p_pie,
  paste0("pie_all_conditions_", cells, ".png"),
  width = 10,
  height = 4
)
```
...
```{r save-rds}
saveRDS(spe, file.path(params$output,"spe_PT_spatial.rds"))
```

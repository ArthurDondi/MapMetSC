...
for (cells in cells_list) {
  ...
  for (cluster_name in order) {
    ...
    stats_df <- lapply(comparison_pairs, function(pair) {
      ...
      data.frame(group1 = g1, group2 = g2, p_adj = p_val)
    }) %>% dplyr::bind_rows()

    stats_df$p_adj <- p.adjust(stats_df$p_adj, method = "BH")
    stats_df$p_adj_signif <- stars.pval(stats_df$p_adj)
    stats_df$p_adj_signif[stats_df$p_adj_signif == " "] <- "ns"
    stats_df$label <- paste0(round(stats_df$p_adj, 3), " (", stats_df$p_adj_signif, ")")

    y_max <- max(proportions$Freq, na.rm = TRUE)
    if (!is.finite(y_max)) y_max <- 0
    stats_df$y.position <- y_max * c(1.10, 1.20, 1.30) + 0.01
